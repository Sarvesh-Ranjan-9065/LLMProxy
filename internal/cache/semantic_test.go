...
func TestNormalizeStringCaseSensitiveField(t *testing.T) {
	h := NewSemanticHasher()

	got := h.normalizeString("function_name", "MyFunction", true)
	if got != "MyFunction" {
		t.Errorf("expected case preserved for function_name, got %s", got)
	}

	model := h.normalizeString("model", "GPT-4o", true)
	if model != "gpt-4o" {
		t.Errorf("expected model lowercased, got %s", model)
	}
}
